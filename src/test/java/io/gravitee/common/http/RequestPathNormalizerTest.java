...
class RequestPathNormalizerTest {
    ...
    @Nested
    class Cost_on_long_paths {

        // Far past anything a server would accept on a request line. The point is precisely that this
        // class does not rely on a server being in front of it: it is a library, any caller can hand
        // it any string, so the cost has to hold on its own.
        private static final int ESCAPES = 350_000;

        // Both tests run under a deadline rather than merely asserting their result. Decoding by
        // deleting in place shifts the suffix behind every escape, and at this size that quadratic
        // term does not just make the tests slow, it makes them never finish: measured at over eight
        // minutes against a linear pass that does both in forty milliseconds. Without the deadline a
        // regression would hang the build instead of failing it. Two seconds is loose on purpose —
        // it is there to catch the reintroduction of the quadratic term, not to measure anything.
        private static final Duration BUDGET = Duration.ofSeconds(2);

        @Test
        void should_decode_a_path_of_escapes_in_linear_time() {
            final String path = "/" + "%41".repeat(ESCAPES);

            final String normalized = assertTimeoutPreemptively(BUDGET, () -> RequestPathNormalizer.normalize(path));

            assertThat(normalized).isEqualTo("/" + "A".repeat(ESCAPES));
        }

        @Test
        void should_leave_reserved_escapes_encoded_on_a_long_path() {
            // Same pass, the branch that writes three characters back out. Mixed with decodable ones
            // so the write cursor is behind the read cursor by the time each %2F is copied over.
            final String path = "/" + "%41%2F".repeat(ESCAPES);

            final String normalized = assertTimeoutPreemptively(BUDGET, () -> RequestPathNormalizer.normalize(path));

            assertThat(normalized).isEqualTo("/" + "A%2F".repeat(ESCAPES));
        }
    }
